...
if option == "Sequence": # search by name, GenBank accession number
    ...
    c1.markdown("*Example: AB020851.2 1-100 accession sequence*")
    
# improve 
    c2 = st.container()
    c2.subheader("Protein sequence")
    c2.write("[taxon] [Protein name] (start-end) protein sequence")
    c2.markdown("*Example: human Deleted in malignant tumor 1 1-100 protein sequence*")
    
    c2.write("[GenBank accession number] (start-end) protein sequence")
    c2.markdown("*Example: AB020851.2 1-100 protein sequence*")
    
    c3 = st.container()
    c3.subheader("Gene coding sequence")
    c3.write("[GenBank accession number] (start-end) gene coding sequence") # check start_end
    c3.markdown("*Example: AB020851.2 1-100 accession coding sequence*") 
    
    c4 = st.container()
    c4.subheader("Protein coding sequence")
    c4.write("[taxon] [Protein name] (start-end) protein coding sequence")
    c4.markdown("*Example: human Deleted in malignant tumor 1 1-100 protein coding sequence*")
